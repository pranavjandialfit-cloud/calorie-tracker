...
/**
 * Calorie Tracker — Apple-like single-file React app (v0.3)
 * -----------------------------------------------------------
 * What's new in v0.3
 * - Premium feel: toned-down neutrals, soft depth, clearer contrast
 * - "Log Your Meal" compact card → opens a sheet-style composer (modal)
 * - Composer hosts the previous add-meal form (with fiber)
 * - Same rings/targets, localStorage, import/export
 */

// ---------- Utilities ----------
const LS_KEY = "ct_entries_v3"; // bumped key (styling changes ok to reuse data; we migrate below)
  ...
  return (
    <div className={`flex flex-col items-center gap-2 ${className}`}>
      ...
      <div className="text-center">
        ...
        <div className="text-lg font-semibold tracking-tight">{Math.round(value)} / {Math.round(max)}</div>
      </div>
    </div>
  );
}
...
/** @typedef {{ id:string, date:string, time:string, title:string, photo?:string, calories:number, protein:number, carbs:number, fat:number, fiber:number, notes?:string }} Entry */
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      // migrate older keys
      const keys = ["ct_entries_v2", "ct_entries_v1"];
      for (const k of keys) {
  ...
  function duplicateEntry(entry) { const { id, ...rest } = entry; addEntry(rest); }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f7f8fb] to-[#eef2f7] dark:from-neutral-950 dark:to-neutral-900 text-neutral-900 dark:text-neutral-100 font-sans selection:bg-black selection:text-white">
      ...
      <main className="mx-auto max-w-3xl px-4 pb-28">
        ...
        <MealComposer open={composerOpen} onClose={() => setComposerOpen(false)} onCreate={(e) => { addEntry(e); setComposerOpen(false); }} />
        ...
        <section className="mt-6">
          ...
          <div className="space-y-4">
            {dayEntries.length === 0 ? (
              ...
              dayEntries.map((e) => (
                <EntryCard key={e.id} entry={e} onUpdate={updateEntry} onDelete={() => removeEntry(e.id)} onDuplicate={() => duplicateEntry(e)} />
              ))
            )}
          </div>
        </section>
      ...
      <BottomBar onToday={() => setActiveDate(todayKey())} onExport={() => downloadJSON(entries)} onImport={(data) => setEntries(data)} />
    </div>
  );
}
    ...
    <header className="sticky top-0 z-20 backdrop-blur-xl bg-white/70 dark:bg-neutral-900/60 border-b border-black/5 dark:border-white/10">
      <div className="mx-auto max-w-3xl px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-black/50 dark:text-white/50">
          ...
          <span className="px-2 py-1 rounded-full border border-black/10 dark:border-white/10">v0.3</span>
        </div>
      </div>
    </header>
  );
  ...
  return (
    <section className="mt-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="i-btn" onClick={() => setActiveDate(prevDay(activeDate))} aria-label="Previous day">◀︎</button>
          <input ref={ref} type="date" className="i-input !w-[11.5rem]" value={activeDate} onChange={(e) => setActiveDate(e.target.value)} />
          <button className="i-btn" onClick={() => setActiveDate(nextDay(activeDate))} aria-label="Next day">▶︎</button>
        </div>
      </div>
    </section>
  );
...
function MealComposer({ open, onClose, onCreate }) {
  ...
  const [form, setForm] = useState({ title: "", calories: "", protein: "", carbs: "", fat: "", fiber: "", notes: "" });
  ...
  useEffect(() => {
    if (!open) {
      setFile(null);
      setForm({ title: "", calories: "", protein: "", carbs: "", fat: "", fiber: "", notes: "" });
    }
  }, [open]);
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-0 w-full max-w-3xl px-4 pb-4">
        <div className="sheet rounded-t-3xl p-4 sm:p-5">
          <div className="flex items-center justify-between">
            ...
            <button className="i-btn" onClick={onClose}>Close</button>
          </div>
          ...
          <div className="mt-4 flex items-start gap-4">
            <label className="relative w-28 h-28 shrink-0 rounded-2xl overflow-hidden bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/10 flex items-center justify-center cursor-pointer">
              {preview ? (<img src={preview} alt="preview" className="w-full h-full object-cover" />) : (<span className="text-sm text-black/60 dark:text-white/60">Add photo</span>)}
              <input type="file" accept="image/*" className="absolute inset-0 opacity-0" onChange={(e) => setFile(e.target.files?.[0] || null)} />
            </label>

            <div className="grid grid-cols-2 gap-3 flex-1">
              <TextField label="Title" placeholder="e.g., Paneer Bowl" value={form.title} onChange={(v) => setForm((f) => ({ ...f, title: v }))} />
              <NumberField label="Calories" placeholder="kcal" value={form.calories} onChange={(v) => setForm((f) => ({ ...f, calories: v }))} />
              <NumberField label="Protein" placeholder="g" value={form.protein} onChange={(v) => setForm((f) => ({ ...f, protein: v }))} />
              <NumberField label="Carbs" placeholder="g" value={form.carbs} onChange={(v) => setForm((f) => ({ ...f, carbs: v }))} />
              <NumberField label="Fat" placeholder="g" value={form.fat} onChange={(v) => setForm((f) => ({ ...f, fat: v }))} />
              <NumberField label="Fiber" placeholder="g" value={form.fiber} onChange={(v) => setForm((f) => ({ ...f, fiber: v }))} />
              <TextField label="Notes" placeholder="optional" value={form.notes} onChange={(v) => setForm((f) => ({ ...f, notes: v }))} />

              <div className="col-span-2 flex gap-2 pt-1">
                <button className="i-btn primary" onClick={handleAdd} disabled={loading}>Add Entry</button>
                <button className="i-btn" onClick={handleMockAI} disabled={!file || loading}>{loading ? "Estimating…" : "Estimate via AI (mock)"}</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <article className="glass p-4 rounded-3xl">
      <div className="flex items-start gap-4">
        <div className="relative w-24 h-24 shrink-0 rounded-2xl overflow-hidden bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/10">
          {entry.photo ? (<img src={entry.photo} alt={entry.title} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center text-xs text-black/50 dark:text-white/50">No photo</div>)}
        </div>

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 justify-between">
            {editing ? (
              <input className="i-input !py-1 !px-2 !h-9 font-medium" value={draft.title} onChange={(e) => setDraft((d) => ({ ...d, title: e.target.value }))} />
            ) : (
          ...
          <div className="mt-2">
            {editing ? (
              <input className="i-input !py-1 !px-2 !h-9" placeholder="Notes" value={draft.notes || ""} onChange={(e) => setDraft((d) => ({ ...d, notes: e.target.value }))} />
            ) : entry.notes ? (
          ...
          <div className="mt-3 flex items-center gap-2">
            {editing ? (
              <>
                <button className="i-btn primary" onClick={save}>Save</button>
                <button className="i-btn" onClick={() => setEditing(false)}>Cancel</button>
              </>
            ) : (
              <>
                <button className="i-btn" onClick={() => setEditing(true)}>Edit</button>
                <button className="i-btn" onClick={onDuplicate}>Duplicate</button>
                <button className="i-btn danger" onClick={onDelete}>Delete</button>
              </>
            )}
          </div>
        </div>
...
function BottomBar({ onToday, onExport, onImport }) {
    ...
    <footer className="fixed bottom-4 left-0 right-0">
      <div className="mx-auto max-w-3xl px-4">
        <div className="glass rounded-2xl p-2 flex items-center justify-between">
          <button className="i-btn" onClick={onToday}>Today</button>
          <div className="flex items-center gap-2">
            <button className="i-btn" onClick={() => onExport()}>Export JSON</button>
            <input type="file" accept="application/json" ref={fileRef} className="hidden" onChange={async (e) => {
              const file = e.target.files?.[0];
              if (!file) return; const text = await file.text();
              try { const data = JSON.parse(text); if (Array.isArray(data)) onImport(data); }
              catch { alert("Invalid file"); } finally { e.target.value = ""; }
            }} />
            <button className="i-btn" onClick={() => fileRef.current?.click()}>Import</button>
          </div>
        </div>
      </div>
    </footer>
...
function NumberField({ label, value, onChange, placeholder }) {
  return (
    <label className="flex flex-col gap-1 text-sm">
      ...
      <input className="i-input" type="number" inputMode="decimal" placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
    </label>
  );
}
...
function NumMini({ label, value, onChange }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <input className="i-input !py-1 !px-2 !h-9 w-24" type="number" inputMode="decimal" value={value} onChange={(e) => onChange(e.target.value)} />
    </div>
  );
}
...
function Logo() {
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center text-[10px] font-semibold tracking-tight text-black/70 dark:text-white/80">CT</div>
    </div>
  );
}

// ---------- Helpers ----------
function prevDay(iso) { const d = new Date(iso); d.setDate(d.getDate() - 1); return d.toISOString().slice(0, 10); }
function nextDay(iso) { const d = new Date(iso); d.setDate(d.getDate() + 1); return d.toISOString().slice(0, 10); }
function guessTitleFromFile(name = "") { const base = name.split(".")[0].replace(/[_-]+/g, " "); if (!base) return "Meal"; return base.split(" ").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" "); }
function downloadJSON(entries) { const blob = new Blob([JSON.stringify(entries, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `calorie-tracker-${todayKey()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); }
